...
require('./hwkal')
const { default: WebSocket, useSingleFileAuthState, DisconnectReason, fetchLatestBaileysVersion, generateForwardMessageContent, prepareWAMessageMedia, generateWAMessageFromContent, generateMessageID, downloadContentFromMessage, makeInMemoryStore, jidDecode, proto } = require("@adiwajshing/baileys")
const fs = require('fs')
...
const { state, saveState } = useSingleFileAuthState(`./haikal.json`)
    ...
    haikal.ev.on('group-participants.update', async (anu) => {
        ...
        try {
            ...
            for (let num of participants) {
                ...
                if (anu.action == 'add') {
                    haikal.sendMessage(anu.id, { image: { url: ppuser }, mentions: [num], caption: `Selamat Datang Di Group ${metadata.subject} ${metadata.desc} @${num.split("@")[0]}` })
                } else if (anu.action == 'remove') {
                    haikal.sendMessage(anu.id, { image: { url: ppuser }, mentions: [num], caption: `@${num.split("@")[0]} Selamat Tinggal Di Group ${metadata.subject} ${metadata.desc}` })
